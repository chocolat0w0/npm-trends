...
## 7. グラフの値を１週間ごとの数字にまとめる ✅ (2025-12-13 完了)

- `npmClient.ts` で API レスポンスから 1 週間ごとのダウンロード数にまとめて保管する。

## 8. パッケージ更新状況を表示 ✅ (2025-12-14 完了)

- `https://registry.npmjs.org/<package>` API からパッケージの更新状況を取得してダウンロード数グラフの折れ線上にドットで描画する。パッケージ更新は API レスポンスの `time` に `{<version>:<DateTime>, <version>:<DateTime>...}` 形式で表現される。
- オンオフスイッチをグラフ近傍に追加して表示/非表示を切り替えられるようにする。
